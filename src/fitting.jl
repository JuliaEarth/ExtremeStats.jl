...
"""
function fit_mle(::Type{GeneralizedExtremeValue}, bm::BlockMaxima; init=(μ=0.0, σ=1.0, ξ=0.05))
  # retrieve maxima values
  x = collect(bm)
  n = length(x)

  # set up the problem
  mle = Model(optimizer_with_attributes(Ipopt.Optimizer, "print_level" => 0, "sb" => "yes", "max_iter" => 250))
...
"""
function fit_mle(::Type{GeneralizedPareto}, pm::PeakOverThreshold; init=(σ=1.0, ξ=0.05))
  # retrieve maxima values
  x = collect(pm)
  n = length(x)
